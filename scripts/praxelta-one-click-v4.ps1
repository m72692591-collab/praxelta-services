param(
    ...
    [Parameter(Mandatory = $true)][string]$EmbeddedZipPath
)
...
$Log = Join-Path $LogDir 'one-click-v4-install.log'
$Evidence = Join-Path $State 'ONE_CLICK_V4_INSTALL.json'
$Temporary = Join-Path $env:TEMP ('praxelta-v4-install-' + [Guid]::NewGuid().ToString('N'))
$Extract = Join-Path $Temporary 'release'
$Timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$Backup = Join-Path $Documents ('PRAXELTA\Backups\Travel\preinstall-' + $Timestamp)
$Media = Join-Path $Documents 'PRAXELTA\Installation Media\Travel-0.3.0-V4'
...
function Invoke-ChildPowerShell {
    ...
    & powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $File 2>&1 | ForEach-Object {
        $text = [string]$_
...
function Write-Evidence {
    ...
    $payload = [ordered]@{
        ...
        remaining_owner_actions = @(
            'BotFather: create or confirm a Telegram bot and enter its token locally',
            'Travelpayouts: personally accept the current terms and enter account identifiers locally',
...
try {
    ...
    Ensure-Directory -Path $LogDir
    Write-Line -Message 'PRAXELTA Travel one-click V4 installation started.'

    if (-not (Test-Path -LiteralPath $EmbeddedZipPath -PathType Leaf)) {
        throw ('Embedded release ZIP is missing: ' + $EmbeddedZipPath)
    }
    $actual = (Get-FileHash -LiteralPath $EmbeddedZipPath -Algorithm SHA256).Hash.ToLowerInvariant()
    if ($actual -ne $ExpectedInnerSha256) {
        throw ('Embedded release ZIP SHA-256 mismatch: ' + $actual)
    }
    Add-Step -Id 'embedded-release-hash' -Passed $true -Message 'Embedded release ZIP verified.' -Data $actual

    Expand-Archive -LiteralPath $EmbeddedZipPath -DestinationPath $Extract -Force
    ...
    foreach ($required in @(
        ...
        if (-not (Test-Path -LiteralPath (Join-Path $Source $required))) {
            throw ('Release layout is missing: ' + $required)
        }
    }
    Add-Step -Id 'release-layout' -Passed $true -Message 'Release layout verified.'
    ...
    if (Test-Path -LiteralPath $Target -PathType Container) {
        ...
        Add-Step -Id 'previous-state' -Passed $true -Message 'Existing local secrets and state backed up.' -Data $Backup
    }
    ...
    Copy-Item -LiteralPath $SelfPath -Destination (Join-Path $Media 'PRAXELTA_ONE_CLICK_V4.cmd') -Force
    Copy-Item -LiteralPath $EmbeddedZipPath -Destination (Join-Path $Media 'praxelta-travel-ops-0.3.0.zip') -Force
    [IO.File]::WriteAllText(
    ...
    Invoke-ChildPowerShell -File (Join-Path $Target 'scripts\setup_windows.ps1')
    ...
    if ($env:PRAXELTA_CI -ne '1') {
        Invoke-ChildPowerShell -File (Join-Path $Target 'scripts\create_shortcuts.ps1')
        Invoke-ChildPowerShell -File (Join-Path $Target 'scripts\install_automation.ps1')
    }
    Invoke-ChildPowerShell -File (Join-Path $Target 'scripts\write_pending_actions.ps1')
    ...
    if (-not (Test-Path -LiteralPath $Python -PathType Leaf)) {
        throw 'Virtual-environment Python is missing.'
    }
    ...
    Write-Line -Message 'PRAXELTA_OWNER_MACHINE_TECHNICAL_INSTALL_COMPLETED'
    ...
    Write-Host ('Installed at: ' + $Target) -ForegroundColor Green
