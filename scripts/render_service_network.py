...
<section class="safe-promise"><p class="eyebrow">Граница пилота</p><h2>Система пока работает только на синтетических заявках</h2><p>Ни один реальный контакт не передаётся, деньги не списываются, проценты не начисляются к выплате. Эти функции закрыты юридическим и налоговым gate.</p></section>""",
    ...
    category_options = "".join(
        f'<option value="{html.escape(category["code"])}">{html.escape(category["name"])}</option>'
        for category in CATEGORY_REGISTRY
        if category["wave"] == 1
    )
    form = f"""<section class="local-hero"><p class="eyebrow">Тестовая форма · данные не отправляются</p><h1>Опишите задачу, не указывая полный адрес</h1><p class="lead">Форма показывает, какие данные понадобятся в будущем. Сейчас всё остаётся только на экране вашего браузера и исчезает после закрытия страницы.</p></section>
...
<fieldset><legend>Где и что случилось</legend><div class="form-grid"><label>Город<select name="city" required><option value="">Выберите</option><option>Саратов</option><option>Энгельс</option></select></label><label>Район<input name="district" autocomplete="address-level3" required></label><label>Услуга<select name="category" required><option value="">Выберите</option>{category_options}</select></label><label>Оборудование<input name="equipment" placeholder="Например, стиральная машина"></label><label>Марка<input name="brand"></label><label>Модель<input name="model"></label><label>Код ошибки<input name="error_code"></label><label>Срочность<select name="urgency" required><option>Сегодня</option><option>В течение 2–3 дней</option><option>Планово</option></select></label></div><label>Что происходит<textarea name="problem" minlength="12" required placeholder="Опишите наблюдаемые признаки. Не пишите полный домашний адрес."></textarea></label></fieldset>
